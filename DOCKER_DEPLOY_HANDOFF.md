...
```yaml
  atriumdb-api:
    ...
    command: uvicorn tests.mock_api.app:app --host 0.0.0.0 --port 8000
    environment:
...
* **`context: ../atriumdb/sdk`** — builds from this repo's existing
  `sdk/Dockerfile`, unmodified. It installs the SDK with the `testing` extra,
...
runs. And `sdk/.dockerignore` excludes `tests/test_datasets/` but **not**
`tests/`, which is why `tests/mock_api/` is present in the image at all.

## 3. Task 1 — the blocker

`tests/mock_api/sdk_dependency.py` currently reads:
...
**to match this repo's `tests/mock_api/`** rather than the other way round.
...
1. **Statistics is `/cohorts/statistics`, not `/cohort/statistics`.** The
   router is mounted at prefix `/cohorts` in `tests/mock_api/app.py`; the
   dashboard was calling the singular and would have 404'd.
