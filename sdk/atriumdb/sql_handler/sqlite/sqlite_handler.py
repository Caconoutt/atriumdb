...
class SQLiteHandler(SQLHandler):
    ...
    def select_patient_encounters(self, patient_id_list=None, admit_start_ns=None,
        ...
        arg_tuple = ()
        sqlite_select_patient_encounters_query = (
        ...
        if where_clauses:
            sqlite_select_patient_encounters_query += " WHERE " + " AND ".join(where_clauses)

        sqlite_select_patient_encounters_query += " ORDER BY e.start_time ASC"

        with self.sqlite_db_connection() as (conn, cursor):
            cursor.execute(sqlite_select_patient_encounters_query, arg_tuple)
            rows = cursor.fetchall()
        return rows
